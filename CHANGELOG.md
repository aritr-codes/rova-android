...
- Accessibility **visible focus indicators** (WCAG 2.2 AA SC 2.4.7, ADR-0020): new reusable `Modifier.focusHighlight` draws a 2 dp ring while a custom clickable surface holds keyboard/D-pad focus (the default ripple is invisible on the app's dark chrome). Applied to the record bottom-nav items (NAV-03/05), the recovery card CTAs (RECOV-12), the settings rows (SET-03), and the permission chips (SET-16). The SettingsSheet panel (SET-10) follows in a later slice; the Moderate REC-16/WARN-09 focus gaps are deferred.
