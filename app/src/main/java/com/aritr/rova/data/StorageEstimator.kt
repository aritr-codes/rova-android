...
object StorageEstimator {
     ...
     *
     * When [captureTopology] == `"DualShot"`, the peak doubles to account for
     * 2 encoder outputs (portrait + landscape) running in parallel. Default
     * [captureTopology] = `"Single"` preserves the prior single-mode math for
    ...
    fun estimatePeakBytes(
        ...
        val sides = if (captureTopology == "DualShot") 2L else 1L
