...
/**
 * Pure timing helper for the start cue played by
 * [com.aritr.rova.service.RovaRecordingService.beepStart].
 *
 * Cue-bleed bug (2026-06-18, device RZCYA1VBQ2H): `beepStart` awaits the
 * cue's `MediaPlayer` `onCompletion` under a `withTimeoutOrNull` ceiling,
 * then opens the mic. The old ceiling was a fixed 1500 ms, justified by a
 * stale KDoc claiming `R.raw.rova_beep` is "~300 ms". On-device the asset
 * is a ~3527 ms, 4-pulse cue, so the 1500 ms ceiling always tripped ~2 s
 * before `onCompletion` → the await truncated mid-cue → the mic opened
 * while the cue was still playing → the cue bled into the recording.
 *
 * The ceiling must therefore be derived from the *actual* cue duration so
 * the await genuinely reaches `onCompletion`. JVM-unit-tested in
