 ...
 *   lifetime.
 * - [intervalSeconds] — `SessionConfig.intervalSeconds`. `0` means
 ...
 * 2. Else if [audioMode] is `VIDEO_AUDIO` AND [intervalSeconds] is
...
internal fun shouldPlayBeep(
    ...
    if (audioMode == AudioMode.VIDEO_AUDIO && intervalSeconds == 0) return false
    return true
}
