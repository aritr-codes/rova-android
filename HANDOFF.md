...
**Track C — differentiated recording cues (P3, PLAN-READY, effort S, device-mandatory).** Service/audio-only, disjoint from icon+player → fully parallel-safe. **Plan written** `docs/superpowers/plans/2026-06-22-differentiated-recording-cues.md`: recover the pre-#121 long multi-pulse cue as `rova_cue_start` (git blob `14efedcd…`, **112849 bytes**, binary-safe extract — PowerShell `>` corrupts it), `beepStart(intervalMinutes, isFirstSegment)` branches asset (long first cue / short reminder = existing `rova_beep`), delete `beepEnd` (no end cue), fix stale `BeepTiming`/service KDoc. Device-smoke cue identity + no-bleed (single + DualShot). *(BACKLOG → Audio cues; `memory/project_beep_cue_bleed_fix.md`.)*
...
**Do NOT pick (stale/blocked):** Library/History redesign (DONE) · `checkA11y*` suite (DONE, #114) · PR-6 interactive timeline (DONE, #127) · icon 5b-1 foundation (DONE, #128) · icon 5b-2/5b-3/5b-5 (DONE, #130/#131/#132) · PR-δ FrontBack/DualSight (silicon-blocked on RZCYA1VBQ2H) · `UI_ROADMAP.md` (superseded).
