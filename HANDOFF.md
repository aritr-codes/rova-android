...
> **As of 2026-06-23 — differentiated recording cues (#133) + git pre-flight script (#134) MERGED → `master aae66e7`; `origin/master` CURRENT.** #133 = first-segment long multi-pulse start cue (`rova_cue_start`, restored 112849-byte pre-#121 asset) vs short reminder (`rova_beep`) via `beepStart(isFirstSegment)`; `beepEnd` deleted (no end cue); codex CancellationException-rethrow fix; device-GO. #134 = read-only `scripts/preflight.{ps1,sh}` git-hygiene scan (House convention) + CLAUDE.md pointer. Repo hygiene done this session: master rebased+synced to origin, 4 stale local + 4 merged remote branches deleted.
