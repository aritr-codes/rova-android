...
3. **PR-3 Icon foundation** (batched) — ✅ **MERGED 2026-06-19, PR #124 → master `881f8c8`.** 15 board-exact glyphs (search/share/delete/favorite/_on/select/pause/view/edit/theme + warn_tri/notif_bell/_off + `rec_clipcheck`/`interrupted`) authored in `RovaGlyphs`; `IconStatus.Danger` added; `RovaIcons` bridges flipped. (`RecordRing`/`Waiting`/`ProcArc`/`ProcDots` shipped in PR-2.)
4. **PR-4 Library + Vault migration** — ✅ **MERGED 2026-06-19, PR #125 → master `c8350df`.** Whole Library surface + `VaultScreen` → seam; delete→`IconStatus.Danger`; favorite outline→filled; pure `LibraryIconSpec` (JVM-tested). Vault pulled forward from PR-5.
5. **PR-5 Player slice** — ✅ **MERGED 2026-06-20, PR #126.** Scoped to **icons + control-row honesty**: transport (center-play + play/pause toggle) → `RovaIcons.Play`/`Pause` via pure `PlayerIconSpec.transportGlyph` (JVM-tested); **removed the two FAKE Trim/Edit buttons** (snackbar stubs, no VM entry point, editing NO-GO). Back/±10s seek stay stock (identity-only fence). `rec_clipcheck`/`interrupted` glyphs were authored in PR-3 but are **status** concepts → deferred to the remaining-surfaces slice below (not Player).
6. **PR-5b Remaining icon surfaces** (PENDING) — Warnings / Settings / Onboarding migration + wire the authored `rec_clipcheck` (Recovered status) / `interrupted` glyphs into `RovaIcons` + their Recovery/History/notification surfaces. Last icon-system slice.
7. **Player functionality roadmap** (separate track, post-icons — see BACKLOG "Video / Player / Editing"): **PR-6** navigation core (interactive timeline tap+scrub · segment prev/next · wall-clock playhead · resume) → **PR-7** skim/comfort (playback speed · double-tap seek · auto-hide).
