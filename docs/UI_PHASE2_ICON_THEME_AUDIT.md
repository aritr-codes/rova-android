...
1. **PR-1 Theme hygiene** (S, no device): `reduceTransparency` ← real high-contrast-text signal (`Settings.Secure "high_text_contrast_enabled"`) OR reduce-motion (no regression); memoize `PaletteColorScheme.from` via `remember(palette)` at Theme.kt:40 + :89. New pure `ReducedTransparency` seam + JVM test.
2. **PR-2 FAB lifecycle** (M, device-smoke): pure `RecordFabVisualSpec` + `IconRole.OnAccent`; FAB reads `colorScheme.primary`; extend `RecordFabState` → board set {rec_disc, rec_morph, rec_ring, waiting, proc_arc} (Waiting→waiting, Merging→ProcessingGlyph); author `waiting`/`proc_dots`/`rec_morph`; `RecordScreen` action contract; AA across 12 palettes in `TokenContrastTest`.
3. **PR-3 Icon foundation** (batched): author everyday-action glyphs (search/share/delete/favorite/_on/select/pause/view/edit/theme) + status (warn_tri/notif_bell/_off) in `RovaGlyphs`; flip `RovaIcons` bridges. Additive.
...
6. **Backlog** (filed, not scheduled): P2 adaptive scrim over live camera → RecordChromeTokens palette-derivation; Material You 13th theme; true light/dark pairing per accent.
