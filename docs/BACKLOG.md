...
3. **Rova video-player upgrades** (Video/Player/Editing, P2, NEEDS-SPEC) / **DualShot fps cadence cap** (P2, NEEDS-SPEC — the open remainder of DualShot performance after the dead-Quality-lever fix shipped #152; fps plateaus ~18–20 at all presets, upstream `CameraEffect` frame-delivery limiter, not bitrate/resolution) — strongest remaining UI/capture candidates now that the service split is closed and the DualShot bitrate lever is fixed. *(Source: `memory/project_dualshot_preset_honoring.md`; `memory/project_current_state.md`; BACKLOG Capture/Mode.)*
...
- [~] **DualShot performance — frame jitter + thermal buildup** — **P2** · **owner-reported 2026-06-17 (device RZCYA1VBQ2H)** · **profiling DONE + bitrate lever SHIPPED #152 (master `e1eeb0c`, 2026-06-28); fps cadence cap still OPEN**
  Two distinct symptoms from owner DualShot smoke: **(1) frame jitter/stutter** and **(2) excessive heating**. **Profiling pass complete** (device RZCYA1VBQ2H): DualShot ran 18.56 fps vs single 30.02; AP thermal +2.1°C plateau; root of the *waste* was a **dead Quality lever** — both encoders were hard-coded 2×1080p@8Mbps regardless of preset.
  **SHIPPED #152:** DualShot now honors the Quality preset (`DualShotPresetResolver` + `BitrateTable` per side; 4K→FHD cap; crop geometry unchanged). SD now ~4.8MB/side/10s vs old ~80MB/side session → big heat/bandwidth/storage win at lower presets without touching the render topology. FHD device-verified: no new stutter/thermal cliff, dims byte-identical, bitrate 8→16.78 cap as designed.
  **STILL OPEN (next slice):** fps plateaus ~18–20 even at SD — *not* bitrate/resolution bound. Ground-truth `stts` + `EglRouter perf` instrumentation point to an **upstream `CameraEffect` frame-delivery cadence limiter** (render ~1.3ms, idle-bound), not the encoder. Investigate: SurfaceTexture/CameraEffect frame pacing, per-frame copy/FBO cost on the GL thread, whether the broadcast `AudioRecord` + dual-muxer cadence stalls delivery, and whether δ0 (CameraX 1.5.3 SurfaceProcessor-shutdown fix) helps. Needs its own spec. *(Source: owner DualShot smoke 2026-06-17; profiling 2026-06-28 → `docs/superpowers/specs/2026-06-28-dualshot-preset-honoring-design.md`; `memory/project_dualshot_stability_stack.md`; ADR-0009.)*
