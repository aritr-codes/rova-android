...
1. **Enhanced Library & History UI** (UI/UX Modernization) — **the headline / next up** (selected 2026-06-14 as the highest-value DualSight-independent task executable on current hardware). The Library/History surfaces — where users browse *all* their recordings — have had **no visual modernization pass**; only the #100 nav state-retention fix + deferred a11y focus rows. Liquid-Glass redesign: richer thumbnails, date/session grouping, sort + filter, multi-select / batch actions, polished empty + loading states; folds in deferred a11y rows 23/32. **NEEDS-SPEC** (brainstorm → spec → plan). No special hardware. *(Source: owner request 2026-06-14; this file's UI/UX Modernization section.)*
2. **Rova video-player upgrades** (Video/Player/Editing, P2, NEEDS-SPEC) — high-engagement surface, Media3-based, executable on current hardware; folds in a11y timeline-semantics row 21. **or** **`RovaRecordingService.kt` split** (Reliability/Service, P2) — highest-leverage *tech-debt* (largest file; gates pin the invariants), no user value, regression-smoke after. *(Source: this file's Video/Player/Editing + Reliability/Service sections.)*
