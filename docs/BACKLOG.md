...
1. **Enhanced Library & History UI** (UI/UX Modernization) — **IN PROGRESS, FUNCTIONALLY COMPLETE (2026-06-15).** Liquid-Glass redesign Slices 1–5 (foundation, layout, selection/batch, discovery+autoplay, a11y close-out incl. rows 21/23/32) all built + device/TalkBack GO on local-only branch `feat/library-history-selection` (~62 ahead of master, NOT pushed; S1 PR #117 + S2 PR #118 OPEN/stacked). ADR-0030 + 42nd gate `checkLibraryNoManifestWrite`. **NEXT = Library UX/UI POLISH PASS P1–P5** (token foundation → hero → cards → states → motion; raise to Record-screen visual quality) — audit + design brief + per-slice direction in `docs/superpowers/specs/2026-06-15-library-dualshot-rca-and-polish-brief.md`; kickoff prompt in HANDOFF.md. Owner: **NO merge until the full reskin (incl. polish) is done.** *(Source: owner request 2026-06-14/15; the brief doc.)*
   - **DualShot hero grey-strip artifact — ROOT-CAUSED 2026-06-15 = Library render-layer bug (H1).** On-device disambiguation (ffprobe/ffmpeg on RZCYA1VBQ2H) proved source frames clean + encoder exact-aspect → capture-bake (H2) refuted. Cause: hero autoplay static `VideoFrame`(Crop) vs ExoPlayer(ZOOM) diverge in the off-16:9 hero box. **Fix folds into polish Slice P2.** Separate owner-deferred PORTRAIT square-into-9:16 *capture* stretch (`AspectFitMath.kt:359-366`, ADR-0009/0010) is unrelated, stays capture-side. *(Source: the brief doc PART A.)*
