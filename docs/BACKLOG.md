...
- [ ] **Gate-count documentation recount sweep** — **P1**
...
- **Gate count is measured, not estimated:** 39 unique `tasks.register("check…` entries in `app/build.gradle.kts`, all 39 wired into `preBuild` via `dependsOn(...)`. CLAUDE.md's "28"/"25" and the PR-ε memory's "36" are stale.
