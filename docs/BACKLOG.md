...
1. **Icon & Glyph System Redesign** (UI/UX Modernization) — **NEXT SESSION (owner-chosen 2026-06-16), NEEDS-SPEC.** Establish an icon philosophy + glyph language + consistency rules + semantic icon-to-action mapping for the in-app icon set (nav, controls, chrome, action glyphs), forward-compatible with the upcoming Liquid Glass theme engine. The launcher icon was rebranded in M5; this is the in-UI system. Trigger prompt at the bottom of HANDOFF.md. *(Source: owner request 2026-06-16; was "Icon modernization for Liquid Glass" P3 below, now promoted.)*
2. **Liquid Glass theme engine** (UI/UX Modernization) — the deferred big one. Library is now the **reference screen** for the glass tint engine (Theme-Foundation M1–M3 wired `LibraryColors`/`LibraryColorSpec` seam, device GO). Follows the icon session. *(Source: owner sequencing 2026-06-16; ADR-0028.)*
3. **End-of-reskin merge train** (process) — land the ~91-commit `feat/library-history-selection` branch to master. Owner-gated; strategy must account for #117/#118 covering only Slices 1–2 (~88 commits on no pushed branch). See the Merge-train note below + `memory/feedback_stacked_pr_merge_train.md`. *(Source: owner directive; this session's stabilization closeout.)*

> **Enhanced Library & History UI — ✅ COMPLETE 2026-06-16.** Liquid-Glass redesign: Slices 1–5 (foundation, layout, selection/batch, discovery+autoplay, a11y rows 21/23/32) + polish **P1–P8** + Theme-Foundation **M1–M3** + M1/M2 UX refine + stabilization (sort-crash fix `e0f359a`). All built + device/TalkBack GO, local-only on `feat/library-history-selection` (~91 ahead of master; S1 PR #117 + S2 PR #118 OPEN/stacked, cover Slices 1–2 only). ADR-0030 + 42nd gate `checkLibraryNoManifestWrite`. DualShot hero grey-strip artifact FIXED in P2 (TextureView; render-layer bug, capture refuted). Reference-ready for the theme engine. Remaining = the owner-gated merge train + deferred B-items (top-bar action reorder, hero "Latest" tag, item-sheet Play emphasis, batch-Delete isolation, scrubber-thumb rest-state refinement). *(Source: `memory/project_library_history_redesign.md`; this session.)*
...
- [ ] **Icon & Glyph System Redesign** — **P1 (next session, owner-chosen 2026-06-16)** · **NEEDS-SPEC**
  Recreate/modernize the in-app icon + glyph set to match the Liquid Glass design language and visual weight. Establish: icon **philosophy**, **glyph language**, **consistency rules**, **semantic icon→action mapping**, and **forward compatibility with the upcoming Liquid Glass theme engine**. The launcher icon was already rebranded in M5; this is the in-UI icon system (nav, controls, chrome, action glyphs) for consistency. Trigger prompt at the bottom of HANDOFF.md. **Note (measured this session):** the app uses `androidx.compose.material.icons` (`material-icons-extended`) — a **pinned** set; some desired glyphs (e.g. CropPortrait/Landscape) are absent, which already forced custom-drawn pills (`OrientationFramePill`). The icon system must decide build-vs-pinned and a custom-vector home. *(Source: owner request 2026-06-16; Liquid Glass track ADR-0028; launcher rebrand in `memory/project_current_state.md` M5 entry; pinned-icon-set fact from the Library polish pass.)*
