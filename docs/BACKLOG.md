...
> **As of 2026-06-18, master at HEAD `0a8818e`, origin/master CURRENT (pushed through `0a8818e`)** (Liquid Glass theme engine slice 1 / PR #120 + Icon P2 Track A branded merge animation both merged; on top of Icon system P0+P1a + premium dialogs + Library redesign). **46** custom `check*` gates. The **P1 recording-cue audio-bleed bug is FIXED** (PR #121, branch `fix/beep-cue-bleed`, OPEN against master, device-smoke PASS). Open PRs = **#121 cue-bleed fix** (ready to merge) + **#115 DualSight** (DRAFT, archival, never-merge). **NEXT = owner pick from the next-task shortlist below (top backbone candidate = `RovaRecordingService.kt` split; top user-facing = video-player upgrades; owner-reported = DualShot perf jitter/thermal — needs a device profiling pass first).**
...
1. **Recording-cue audio-bleed bug** (Known Bugs) — **NEXT, P1, media-integrity.** Owner-confirmed (playback) on RZCYA1VBQ2H: the `rova_beep` start cue is captured into the saved clip; per-segment intermediate cues overlap the same way. Intended design is a pre-roll cue outside capture (`beepStart` suspends before `recordSegment` opens the mic) but the timing gate is not holding on-device. **Start with `superpowers:systematic-debugging`** — prove the root cause (MediaPlayer output-latency vs onCompletion / audio-focus-route coupling / acoustic echo / mic-open timing) before fixing; DualShot path shares the ordering. Trigger prompt at the bottom of HANDOFF.md. *(Source: owner report 2026-06-18; Known Bugs entry.)*
2. **`RovaRecordingService.kt` split** (Reliability/Service, P2) — the largest file; decompose into seams without changing behavior, with the static-gate suite pinning invariants. Strongest non-UI candidate (and would make the beep-path seams above easier to test). *(Source: `memory/project_current_state.md`.)*
