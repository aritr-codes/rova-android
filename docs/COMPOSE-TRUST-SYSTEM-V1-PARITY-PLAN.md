...
> **✅ MERGED (PR #187 → `b9d26951`, 2026-07-12). As-shipped:**
> - **Pure classifier** `ui/recovery/MergeFailureReason.kt` — closed set `{STORAGE, UNREADABLE, INCOMPLETE, UNKNOWN}`, each with its `@StringRes`. `classify(ExportResult)` is `when`-exhaustive over the sealed `ExportResult` with **no `else`** (a future variant is a compile error — the closure guarantee) and **never inspects a message string** (total function of the typed variant, not the plan-permitted last-resort Throwable-type heuristic). Mapping: `InsufficientStorage`/`SafFolderUnavailable`→STORAGE, `CopyFailed`→UNREADABLE, `MuxFailed`/`RenameFailed`/`PendingInsertFailed`/`FinalizeFailed`/`ManifestWriteFailed`→INCOMPLETE, `UnknownSession`/`Success`→UNKNOWN (fallback).
> - **Seam:** `RecoveryMerger.toRecoveryOutcome` classifies at the last point the typed variant survives and carries it on `RecoveryMergeOutcome.MuxFailed(cause, reason)` — **`reason` has no default**, so every construction site classifies explicitly. `InsufficientStorage` still maps to `RecoveryMergeOutcome.InsufficientStorage` → **CANT_MERGE sheet untouched**.
> - **State:** `RecoveryUiState` gains `@StringRes mergeFailedReasonRes` (additive, defaulted); raw `mergeFailedReason` kept **for logs only** — no longer user-visible. Co-set on failure, co-cleared on Succeeded/ServiceBusy/UnknownSession **and InsufficientStorage** (the last added in review reconciliation, closing a stale-failbox-under-the-sheet edge).
> - **Failed-frame morph (§08, HTML :1629–:1636):** on `failed` (`mergeFailedReasonRes != null && mergeInProgress == null`) the eyebrow → "Merge failed" (**recency preserved**), title → "Couldn't combine your clips", body → the `FailBox`. Retry replaces the primary. The old inline raw-exception `recovery_merge_failed_prefix` Text is **removed** — no raw exception text reachable in the UI.
> - **Failbox parity (`.failbox` :375–:377):** r-sm 10 (`failBoxCornerRadius`), fill `RovaWarnings.hard@.08`, hairline 1dp `hard@.22`, s3 margin/padding, bold "Your clips are safe." at `onSurface` (tHigh). `--sev-hard` is fixed (not `var(--sev)`) → hardcoded `hard` is correct even on amber cards. One merged node, **assertive** live region.
> - **Copy:** 7 strings (eyebrow/title/reassurance + 4 reasons) in `values/` **and** `values-es/`, EN verbatim vs the frozen §08 table; no format args/plurals (SpanishCatalogParityTest safe). **Note the plan said "×4" — the frozen frame morphs eyebrow+title+body together, so 7; HTML authority.**
> - Tests: `MergeFailureReasonTest` (13, RED-proven via `CopyFailed→INCOMPLETE` mutation), `RecoveryViewModelTest` reason-res assertion, `RovaWarningsV3Test` failbox-radius pin. Gate count **48** (no new gate — ADR-0036/0038 precedent), suite **2380/0-0-0**. One independent GO review; reconciled its single Recommended (InsufficientStorage co-clear).
> - **Deferred (tracked):** `recovery_merge_failed_prefix` string is now dead → left for M11 (no opportunistic cleanup). **Device verification** (storage-revoke → STORAGE string → Retry) is the separate mandatory pre-release gate.
...
### M11 — Cleanup: renames, dead tokens, final parity audit ✅ MERGED (#189 → `78183715`, 2026-07-13)
**As-shipped:** mechanical cleanup only, zero behavior/visual/logic change. Renames: `RovaWarningsV3`→`RovaTrustTokens` (+`RovaWarningsV3Test`→`RovaTrustTokensTest`), `WarningTopBannerV3`→`WarningTopBanner`, `WarningSheetV3`→`WarningSheet`. Dead removals (last consumer migrated in M4–M9): tokens `snoozeChipFillAlpha`/`sevChipForegroundAlpha`/`secondaryCtaTextAlpha`, strings `warning_banner_auto_stop_protect`/`recovery_merge_failed_prefix` (both locales, parity held 670=670), files `RecoveryIconSpec.kt`+`RecoveryIconSpecTest.kt`. **Forced build-logic touch (surfaced, unavoidable):** `checkGlassSurfaceRoleUsage`'s filename allowlist `WarningSheetV3.kt`→`WarningSheet.kt` — filename string only, gate regex/logic byte-identical. All surviving `RovaTrustTokens` values byte-identical to the old object. 4 affected `checkA11y*`/glass gates proven RED→GREEN post-rename. No new gate (48), no ADR (38). Suite **2370/0-0-0** (Δ−11 = −10 `RecoveryIconSpecTest` + −1 `secondaryCtaTextAlpha` test). One independent fresh-context falsification review: **GO, zero findings**. Device-verified RZCYA1VBQ2H (record home + History strip, RecoveryCard force-stopped, hard-block WarningSheet with glow + sev-chip + ghost CTA, sheet→chip collapse, signal grant→clear; no crash/ResourceNotFound/NoSuchField). The unrelated `gradle.properties` build tuning was deliberately kept OUT of M11 history. **Trust System V1 Compose transcription COMPLETE — M0–M11 all merged.**
