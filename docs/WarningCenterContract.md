...
| C2.4 Can't merge yet (3-way) | "Can't Merge Yet" | `Escalating` (3-way choice) | partial | Recovery scanner emits `OFFER_DISCARD` for surviving-segments-no-terminal-write shape; today the card has Discard + vendor — the 3rd option ("Merge what was recorded") is not exposed | Recovery card on `history` with **three** actions: Discard / Merge what was recorded / Keep | until user picks one | dismissible only after a choice; clears on action | **Phase 4.3** — `RovaController.recoverAndMerge(sessionId)` reuses existing merger |
...
| C4.4 Merge failed | "Couldn't create final video" | `Hard` (error card) | partial | Today: `Terminated.USER_STOPPED` with `exportState ∈ {FAILED}` per ADR 0006 §B9 — the manifest classification is correct but no UI mapper exists. Mockup: "Merge failed" framing applied to that classification | Recovery card on `history` (no `MERGE_FAILED` enum value — ADR 0006 §3.3 confirms no manifest schema change needed) | until user discards | dismissible | n/a — Phase 4 owns the UI mapper only |
