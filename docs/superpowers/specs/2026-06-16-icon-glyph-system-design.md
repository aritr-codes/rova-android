...
> **P1a Slice 3 landed 2026-06-17** (`…-icon-system-p1a-slice3.md`): the glyphs go live. Two filler glyphs authored — `Single` (Auto = one capture frame + core) and `FollowDevice` (phone + auto-rotate arrows; the arrows live in the **mono outline** layer, their only differentiator from Portrait, so they never wash out — speaker bar is the duotone accent). Wired through the `SemanticIcon` seam into: the capture-topology picker (`ModeTabs`: Single/DualShot/DualSight) and orientation picker (`OrientationRow`: FollowDevice/Portrait/Landscape) — a glyph above each tab label via pure `captureGlyphFor`/`orientationGlyphFor` mappers; the **Vault** library-item row (new `RovaGlyph` `SheetRow` overload, was `Icons.Filled.Lock`); the **Merge-complete** card (was `Icons.Default.CheckCircle`); and the **Recovery** card (new leading emblem, role=Secondary so it stays subordinate to the severity tag). **`BackgroundRecord` deferred** — the foreground-service notification uses `setSmallIcon(resId)`, a drawable-resource int, so wiring its glyph needs a vector-drawable XML export pipeline (a separate slice), not the `ImageVector` seam. No new gate, string, or schema change.
