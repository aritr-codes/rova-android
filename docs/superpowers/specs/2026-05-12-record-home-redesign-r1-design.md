...
|---|---|---|---|
| Hard block | `CAMERA_PERMISSION_DENIED`, `EXACT_ALARM_DENIED`, `STORAGE_INSUFFICIENT` | red-accented `WarningSheet` (handle, tinted icon, title, body, **primary** CTA, **secondary** "Not now") | auto-presents when the Record screen is shown with a hard-block active; the Start FAB goes `Disabled` **only for the `gatesStart` rows** (`CAMERA_PERMISSION_DENIED` / `STORAGE_INSUFFICIENT`) — `EXACT_ALARM_DENIED` shows the same red modal but does **not** gate Start (Phase-4.1 decision; `hardBlockActive` / `recordFabState` in §2.2 read the camera/storage leaf signals, not exact-alarm); Library/Settings stay reachable (those nav items dim only during an active session, not under a hard-block); "Not now" collapses the sheet to a small glass chip under the status pill (tap → re-opens); primary CTA → `launchActionTarget` (App Settings / exact-alarm settings / App Settings-for-storage) |
