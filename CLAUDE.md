...
`app/build.gradle.kts` registers **46 custom `check*` tasks** wired into `preBuild` via `pluginManager.withPlugin("com.android.application") { tasks.matching { it.name == "preBuild" }.configureEach { dependsOn(...) } }`. Each is a regex/AST scan that enforces an invariant from a specific ADR clause:
