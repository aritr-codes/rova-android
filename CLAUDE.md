...
Native 4:3 source + matrix-driven 27/64 side-crops (ADR-0009). `service/dualrecord/` does EGL14/GLES20 fan-out from a single `CameraEffect(target=PREVIEW)` into a single broadcast `AudioRecord` plus two `MediaMuxer` muxes. Pure JVM tests on `AspectFitMath`, `BitrateTable`, `DualMuxerStateMachine`, `RotationCalculator`, `SegmentPathBuilder`, `AudioFanOutPlanner`. Pause/resume is **deferred**. Mode picker tabs: Portrait / Landscape / P+L. **DualShot honors the Quality preset** (#152 → master `e1eeb0c`): `DualShotPresetResolver.forPreset(raw)` maps the preset → per-side dims (SD 540×960·960×540 / HD 720×1280·1280×720 / FHD 1080×1920·1920×1080; 4K→FHD cap; unknown/null→FHD), and `setupDualCamera` derives per-side bitrate from `BitrateTable.forSize` — replacing the old hard-coded 2×1080p@8Mbps. Crop geometry unchanged (only output resolution/bitrate vary); primitive-Int `forDimensions` for JVM-testability, `android.util.Size` only at the `forPreset` edge. Device-verified SD+FHD on RZCYA1VBQ2H. **Known follow-up (separate slice):** DualShot fps plateaus ~18–20 (upstream `CameraEffect` cadence limiter), does not climb to 30 — bitrate/resolution lever now correct but the cadence cap is unaddressed.
